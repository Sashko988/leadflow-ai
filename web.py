...
"""
LANDING_TEMPLATE_V2 += """<script>let target=0,current=0;window.addEventListener('scroll',()=>{target=window.scrollY*.035},{passive:true});function drift(){current+=(target-current)*.08;document.body.style.setProperty('--parallax-y',(-current).toFixed(2)+'px');requestAnimationFrame(drift)}drift();</script>"""
